...
export class ElasticSearchServiceImplementation extends ElasticSearchService {
  ...
  public static getInstance(
    _repository: ElasticSearchRepository
  ...
  async insertIntoElasticSearch(
    elasticSearchDocument: ElasticSearchDocument
  ): Promise<void> {
    const doc = elasticSearchDocument.firebaseDocId;
    await this._repository.insertIntoElasticSearch(
      this.index,
      doc,
  ...
  async getAutocompleteByPhrasePrefix(
    prefix: string
  ): Promise<ElasticSearchDocument[]> {
    const response = await this._repository.getAutocompleteByPhrasePrefix(
      this.index,
      prefix
    );
    return response.data.hits.hits.map(
      (i: { _source: ElasticSearchDocument }) => i._source
    );
  }
}
