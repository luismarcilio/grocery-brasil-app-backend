...
export class PurchaseServicesImplementation extends PurchaseServices {
  ...
  ): PurchaseServicesImplementation {
    if (!this.instance) {
  ...
  async updatePurchaseThumbnails(purchase: Purchase): Promise<void> {
    ...
    if (!userDocId) {
      throw new PurchaseServicesException(
        `userDocId is undefinned: ${JSON.stringify(purchase)}`
      );
    }
    if (!purchaseDocId) {
      throw new PurchaseServicesException(
        `purchaseDocId is undefinned: ${JSON.stringify(purchase)}`
      );
    }
    await this._repository?.updatePurchase({
      purchase,
      userDocId,
      purchaseDocId,
    });
  ...
  private async _saveNfItems(purchase: Purchase) {
    ...
    purchase.purchaseItemList.forEach((item) => {
      ...
      returnStatuses.push(
        productServices.saveProductPurchase({
  ...
  private _getIdsFromPurchase(
    purchase: Purchase
  ): { userDocId: string | undefined; purchaseDocId: string | undefined } {
  ...
  private async _saveNfFull(purchase: Purchase) {
    ...
    await this._repository?.setFullPurchase({
      purchase,
      userDocId,
      purchaseDocId,
    });
  ...
  private async _saveNfResume(purchase: Purchase) {
    ...
    await this._repository?.setResumePurchase({
      purchaseResume,
      userDocId,
      purchaseDocId,
    });
